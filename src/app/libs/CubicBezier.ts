...
/**
 * Typescript Math Toolkit: Cubic Bezier with arc-length parameterization
 *
...
export class TSMT$CubicBezier extends PlanarCurve
{
  ...
  protected _c3X: number = 0;
  protected _c3Y: number = 0;
  ...
  /**
   * Construct a new TSMT$CubicBezier
   *
   * @returns {nothing}
   */
  constructor()
  ...
  public fromObject(control: IControlPoints): void
  {
    ...
    this.cx  = control.cx;
    this.cy  = control.cy;
    this.cx1 = control.cx1;
    this.cy1 = control.cy1;
    this.x1  = control.x1;
    this.y1  = control.y1;
  }
   ...
   *
   * @returns {Array<number>} - t1 and t2 are the computed natural parameter values for the interior geometric constraints,
   * i.e. (cx, cy) and (cx1, cy1).  The internal geometric constraints of the cubic Bezier are computed
   * such that the curve now interpolates the four input points.  No action is taken if the length of the array
   * is less than 4.
   */
  public interpolate(points: Array<IPoint>): Array<number>
  {
    ...
    const p0: IPoint = points[0];
    const p1: IPoint = points[1];
    const p2: IPoint = points[2];
    const p3: IPoint = points[3];
    ...
    let p: IPoint = solve2x2(a11, a12, a21, a22, b1, b2);
   ...
   * @returns {number}
   */
  public getTAtS(_s: number): number
  {
    ...
    const invFcn: IEvalFcn = (t: number): number => {
      return this._integral.eval((t: number) => this.__integrand(t), 0, t, 8)/this._length - s;
    };


    // name of the game is to quickly get an interval containing the arc length, then let TWBRF do its work;
   ...
   * must exist t in [0,1] such that Bx(t) = _x.
   *
   * @returns {Array<number>} Natural parameters at the specified x-coordinate.  This return array contains either one,
   * two, or three t-values.  There are issues with curves that are exactly or nearly (for numerical purposes)
   * vertical in which there could theoretically be an infinite number of
   * y-coordinates for a single x-coordinate.  This method does not work in such cases, although compensation
