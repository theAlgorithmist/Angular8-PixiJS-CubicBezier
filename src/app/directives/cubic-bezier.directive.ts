...
/**
 * This directive renders the cubic Bezier using PixiJS V4 into a DIV.
 *
...
@Directive({selector: '[cubic-bezier]'})
export class CubicBezierDirective implements OnInit, OnDestroy
{
  // static PIXI options
  ...
  // reference to the Typescript Math Toolkit Cubic Bezier (handles all bezier computations while PIXI handles rendering)
  protected _bezier: TSMT$CubicBezier;
  ...
  /**
   * Indicate that arc length of the Bezier curve has been updated
   *
   * @type {number}
   */
  @Output('length')
  ...
  protected _curve: PIXI.Graphics;                        // cubic bezier curve
  ...
  protected _p2: PIXI.Graphics;   // (cx1, cy1)
  protected _p3: PIXI.Graphics;   // (x1, y1)

  // for 4-point cubic bezier interpolation
  protected _pI1: PIXI.Graphics;  // middle interpolation point 1 (cubic passes through PO, PI1, PI2, and P3)
  protected _pI2: PIXI.Graphics;  // middle interpolation point 2 (cubic passes through PO, PI1, PI2, and P3)

  /**
   * Construct a new {CubicBezierDirective}
   *
  ...
  constructor(protected _elRef: ElementRef)
  {
    ...
    this._bezier = new TSMT$CubicBezier();
  }
  ...
  {
    const options = Object.assign({width: this._width, height: this._height}, CubicBezierDirective.OPTIONS);
    ...
    if (this._pointCount < 4)
    {
      if (!isNaN(x) && !isNaN(y))
      {
        ...
        switch (this._pointCount)
        {
          ...
          // first middle interpolation point
          case 2:
            this._pI1.x = x - this._rect.left;
            this._pI1.y = y - this._rect.top;
          break;

          // second middle interpolation point
          case 3:
            this._pI2.x = x - this._rect.left;
            this._pI2.y = y - this._rect.top;
          break;

          // second endpoint
          case 4:
            this._p3.x = x - this._rect.left;
            this._p3.y = y - this._rect.top;
          break;
        }
        ...
        // point count is zero-based
        if (this._pointCount == 4)
        {
          // don't listen for click events any more (so that control/interp points may be dragged)
          this._elRef.nativeElement.removeEventListener('click', this._clickFcn);

          // 4-point cubic bezier interpolation (note that we are not currently doing anything with the t1/t2
          // values that are returned, but log them to the console to see how the interp. works)
          const t: Array<number> = this._bezier.interpolate([
            {x: this._p0.x, y: this._p0.y},
            {x: this._pI1.x, y: this._pI1.y},
            {x: this._pI2.x, y: this._pI2.y},
            {x: this._p3.x, y: this._p3.y}
          ]);

          console.log('middle interpolation parameters: ', t[0], t[1]);
  ...
  protected __update(): void
  {
    ...
    this._curve.clear();
    this._curve.lineStyle(2, CubicBezierDirective.BEZIER_LINE_COLOR, 1);

    this._curve.moveTo(this._bezier.x0, this._bezier.y0);
    this._curve.bezierCurveTo(this._bezier.cx,
                              this._bezier.cy,
                              this._bezier.cx1,
                              this._bezier.cy1,
                              this._bezier.x1,
    ...
    this._constraints.clear();
    this._constraints.lineStyle(2, CubicBezierDirective.CONSTRAINT_LINE_COLOR, 1);
    this._constraints.moveTo(this._bezier.x0, this._bezier.y0);
    this._constraints.lineTo(this._bezier.cx, this._bezier.cy);
    this._constraints.lineTo(this._bezier.cx1, this._bezier.cy1);
   ...
   *
   * @private
   * @returns {nothing} Draws the circle at the specified location with the specified fill color into the
   * supplied graphic context
   */
  ...
  protected __setup(): void
  {
    ...
    this._p3  = new PIXI.Graphics();
    this._pI1 = new PIXI.Graphics();
    this._pI2 = new PIXI.Graphics();

    this.__createCircle(this._p0);
    this.__createCircle(this._p1);
    this.__createCircle(this._p2);
    this.__createCircle(this._p3);
    this.__createCircle(this._pI1);
    this.__createCircle(this._pI2);

    this.__makeDraggable(this._p0);
    this.__makeDraggable(this._p1);
    this.__makeDraggable(this._p2);
    this.__makeDraggable(this._p3);
    this.__makeDraggable(this._pI1);
    this.__makeDraggable(this._pI2);

    this._control.addChild(this._p0);
    this._control.addChild(this._p1);
    this._control.addChild(this._p2);
    this._control.addChild(this._p3);
    this._control.addChild(this._pI1);
    this._control.addChild(this._pI2);
  ...
  {
    g.lineStyle(1, CubicBezierDirective.CIRCLE_LINE_COLOR, 1);
    g.beginFill(CubicBezierDirective.CIRCLE_FILL_COLOR);
